...
class GetScorersListTest {
    @Mock
    private lateinit var repository: PlayerRankingsRepositoryImpl

    private lateinit var useCase: GetScorersList
    ...
    fun setUP() {
        ...
        useCase = GetScorersList(repository, PostExecutorThreadTest(testScheduler))
    }
    ...
    fun testValidData() {
        ...
        Mockito.`when`(repository.getScorers()).thenReturn(Flowable.just(list))
        val subscriber: TestSubscriber<List<Rankings>> = TestSubscriber.create()

        useCase.playerRankinsRepository.getScorers().subscribe(subscriber)
