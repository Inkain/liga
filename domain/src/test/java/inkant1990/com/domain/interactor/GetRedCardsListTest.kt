...
class GetRedCardsListTest {
    @Mock
    private lateinit var repository: PlayerRankingsRepositoryImpl

    private lateinit var useCase: GetRedsCardsList
    ...
    fun setUP() {
        ...
        useCase = GetRedsCardsList(repository, PostExecutorThreadTest(testScheduler))
    }
    ...
    fun testValidData() {
        ...
        Mockito.`when`(repository.getReds()).thenReturn(Flowable.just(list))
        val subscriber: TestSubscriber<List<Rankings>> = TestSubscriber.create()

        useCase.playerRankinsRepository.getReds().subscribe(subscriber)
